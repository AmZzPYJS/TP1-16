...
1. Sur la forge, créez le dépôt (_repository_) `SimpleFraction`;
   ...
   > En cochant au moins une de ces cases 'Initialize this repository with', le dépot est crée avec un premier commit contenant des fichiers de base comme README, gitignore...
   Si on ne coche aucune case, le dépot est créé vide sans aucun commit.
...
1. Localement, configurez `git` avec votre nom (`user.name`) et votre email (`user.email`) (cf. [Personnalisation de Git](https://git-scm.com/book/fr/v2/Personnalisation-de-Git-Configuration-de-Git));
    ```bash
   git config --global user.name "Mezouer Amîn"
   git config --global user.email "amezouer@gmail.com"
    ```
...
1. Quel OS et quel shell de commande utilisez-vous ?
   > Windows 11 avec Git bash pour pouvoir utilise
   > r les commandes Linux/Unix (grep, find, ect..)
